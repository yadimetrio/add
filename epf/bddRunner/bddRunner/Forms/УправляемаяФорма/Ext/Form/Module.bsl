...
Процедура ПодготовитьСценарийКВыполнению() Экспорт
	...
	КаталогВременногоFeature = ВременныйФайл.Путь + "vanessa-add";

	ИмяВременногоFeature = КаталогВременногоFeature + Разделитель + "temp.feature";

	ЗТ = Новый ЗаписьТекста(ИмяВременногоFeature, КодировкаТекста.UTF8);
...
Функция ПолучитьЗначениеДляПередачиВМетод(Знач Значение, Знач Тип)
	Если Тип = "Строка" Тогда
		РезЗначение = СобратьСтрокуДляПередачиВКод(Значение);
